...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    // we are going to place six spikes
    for (var i = 0; i < 6; i++) {
  ...
  placeSpike(spike, quadrant) {
    ...
    var randomAngle = Phaser.Math.Angle.WrapDegrees(
      Phaser.Math.Between(quadrant * 90, (quadrant + 1) * 90)
    );
    ...
    var spikeX =
      this.bigCircle.x +
      (gameOptions.bigCircleRadius - Phaser.Math.Between(4, 25)) *
        Math.cos(randomAngleRadians);
    var spikeY =
      this.bigCircle.y +
      (gameOptions.bigCircleRadius - Phaser.Math.Between(4, 25)) *
        Math.sin(randomAngleRadians);
    spike.x = spikeX;
    spike.y = spikeY;
      ...
      this.spikeGroup.children.iterate(function(spike) {
        ...
        if (spike.approaching) {
          ...
          if (
            ...
            // create an explosion with 50 particles at player position
            emitter.explode(50, this.player.x, this.player.y);
